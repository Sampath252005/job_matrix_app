...
import {
  ...
  createQuestionReplyService,
  createQuestionService,
  getJobQuestionsService,
  getQuestionRepliesService,
} from "./question.service.js";

interface creatquestionBody{
          question:string,
...
export const createQuestion = async (
  req: Request<{jobId:string},{},creatquestionBody>,
  res: Response,
) => {
  try {
    ...
    if (
      ...
      return res.status(403).json({
        success: false,
        message:
          "Only candidates can post questions",
      });
    }
...
export const getJobQuestions = async (
  req: Request<{jobId:string},{},creatquestionBody>,
  res: Response,
) => {
  try {
...
export const createQuestionReply = async (
  req: Request<{questionId:string},{},{message:string}>,
  res: Response,
) => {
  try {
    ...
    const reply =
      await createQuestionReplyService(
        questionId,
        req.user.id,
        req.user.role,
        req.accessToken,
...
export const getQuestionReplies = async (
  ...
  try {
    ...
    const replies =
      await getQuestionRepliesService(
        questionId,
        req.user.id,
        req.user.role,
        req.accessToken,
  ...
  try {
    ...
    if (
      ...
      return res.status(403).json({
        success: false,
        message:
          "Only recruiters can close questions",
      });
    }
