...
export const createInterview = async (
  ...
  try {
    ...
    io.to(`user:${interview.candidate_id}`).emit(
      "interview:scheduled",
      interview,
    );
    ...
    console.error(
      "Create interview controller error:",
      error,
    );
...
export const getInterviewToken = async (
  ...
  try {
    const result =
      await generateInterviewTokenService(
        interviewId,
        userId,
        userEmail,
        token,
    ...
    return res.status(200).json({
      success: true,
      message:
        "Interview token generated successfully",
      data: {
...
export const updateInterviewStatus =
  async (
    ...
    try {
      const interview =
        await updateInterviewStatusService(
          interviewId,
          recruiterId,
          token,
          status,
...
export const getRecruiterInterviews =
  async (
    ...
    try {
      const interviews =
        await getRecruiterInterviewsService(
          recruiterId,
          token,
        );
      ...
      const message = getErrorMessage(
        error,
        "Unable to fetch recruiter interviews",
      );
