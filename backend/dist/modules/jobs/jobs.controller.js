...
export const getJobId = async (req, res) => {
    try {
        ...
        const { data, error } = await JobsServices.getJobById(recruiter_id, token, jobId);
        if (error) {
            return res
                .status(400)
...
//get all my job
export const getAllMyJob = async (req, res) => {
    try {
        const token = req.accessToken;
        const user = req.user;
        if (!user || !token) {
...
export const updateJobById = async (req, res) => {
    try {
        ...
        const { title, description, location, type, salary, experience } = req.body;
        const { data, error } = await JobsServices.updateJob(jobId, recruiter_id, {
            title,
            description,
            location,
            type,
...
export const deleteJobById = async (req, res) => {
    try {
        ...
        const { data, error } = await JobsServices.deleteJob(jobId, recruiter_id, token);
        if (error) {
            return res
                .status(400)
...
export const closeJobController = async (req, res) => {
    try {
        ...
        const { data, error } = await JobsServices.closeJob(jobId, recruiter_id, token);
        if (error) {
            return res
                .status(400)
...
export const fetchAllPostedJobs = async (req, res) => {
    try {
        const token = req.accessToken;
        if (!req.user || !token) {
            return res.status(401).json({ message: "Unauthorized" });
...
export const getJobDetails = async (req, res) => {
    try {
        ...
        const { data, error } = await JobsServices.GetJobDetailsFromDb(jobId, token);
        if (error) {
            console.error(error);
