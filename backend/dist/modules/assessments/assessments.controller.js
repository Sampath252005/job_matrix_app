...
export const createAssessment = async (req, res) => {
    try {
        ...
        const { job_id, title, description, duration_minutes, passing_score } = req.body;
...
export const getAssessmentByJobController = async (req, res) => {
    try {
        const token = req.accessToken;
        const user = req.user;
        if (!user || !token) {
        ...
        const { data, error } = await AssessmentServices.getAssessmentByJob(jobId, user.id, token);
        if (error) {
            return res.status(400).json({
...
export const getAssessmentByIdController = async (req, res) => {
    try {
        ...
        const { data, error } = await AssessmentServices.getAssessmentById(assessmentId, user.id, token);
        if (error) {
...
export const updateAssessmentController = async (req, res) => {
    try {
        ...
        const { data, error } = await AssessmentServices.updateAssessment(assessmentId, user.id, allowedPayload, token);
        if (error) {
            return res.status(400).json({
...
export const deleteAssessmentController = async (req, res) => {
    try {
        ...
        const assessmentId = req.params.assessmentId;
        const { data, error } = await AssessmentServices.deleteAssessment(assessmentId, user.id, token);
        if (!data) {
            return res.status(400).json({
...
export const startAssessmentController = async (req, res) => {
    try {
        ...
        const result = await AssessmentServices.startAssessment(assessmentId, user.id, token);
        if (result.error) {
            return res.status(400).json({
...
export const saveAnswerController = async (req, res) => {
    try {
        ...
        const { data, error } = await AssessmentServices.saveAnswer(attemptId, user.id, question_id, selected_answer, token);
        if (error) {
...
export const submitAssessmentController = async (req, res) => {
    try {
        ...
        const result = await AssessmentServices.submitAssessment(attemptId, user.id, token);
        if (result.error) {
...
export const getAssessmentResultController = async (req, res) => {
    try {
        const token = req.accessToken;
        const user = req.user;
        if (!token || !user) {
        ...
        const result = await AssessmentServices.getAssessmentResult(attemptId, status, token);
        if (result.error) {
            return res.status(400).json(result.error);
        }
        return res.status(200).json({
...
export const getCandidateAssessments = async (req, res) => {
    try {
        ...
        const assessments = await AssessmentServices.getCandidateAssessments(user.id, token);
        return res.status(200).json({
            data: assessments,
        });
    }
...
export const getCandidateAssessmentById = async (req, res) => {
    try {
        const { assessmentId } = req.params;
        const token = req.accessToken;
        const user = req.user;
        ...
        const assessment = await AssessmentServices.getCandidateAssessmentById(assessmentId, token);
        return res.status(200).json({
            data: assessment,
        });
    }
...
export const getAssessmentResultsController = async (req, res) => {
    try {
        ...
        const { assessmentId } = req.params;
        const result = await AssessmentServices.getAssessmentResults(assessmentId, user.id, token);
        if (result.error) {
            return res.status(400).json(result.error);
        }
        return res.status(200).json({
...
export const getAttemptController = async (req, res) => {
    try {
        const token = req.accessToken;
        const user = req.user;
        if (!token || !user) {
        ...
        const data = await AssessmentServices.getAttempt(req.params.attemptId, user.id, token);
        return res.status(200).json({
            data,
        });
    }
