...
function getMonthlyTrend(dates) {
    const now = new Date();
...
export const getRecruiterDashboardAnalytics = async (recruiterId, token) => {
    const supabase = getSupabase(token);
    ...
    if (attemptsError)
        throw attemptsError;
    const submittedAttempts = (attempts ?? []).filter((attempt) => attempt.status !== "STARTED");
    const averageScore = submittedAttempts.length
        ? submittedAttempts.reduce((total, attempt) => total + Number(attempt.score ?? 0), 0) / submittedAttempts.length
        : 0;
    return {
        monthlyApplications: getMonthlyTrend((applications ?? []).map((application) => application.applied_at)),
...
export const getCandidateDashboardAnalytics = async (candidateId, token) => {
    const supabase = getSupabase(token);
    const { data: applications, error: applicationsError } = await supabase
        .from("applications")
